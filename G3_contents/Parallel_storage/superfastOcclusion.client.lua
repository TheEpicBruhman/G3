--[[
    superfastOcclusion (LocalScript)
...
if not script.Parent:IsA("Actor") then
	...
	RunService.PostSimulation:Connect(function()
		...
		for index, child in ipairs(g3Gui) do
			...
			selectedActor:SendMessage("occlusionCheck", child.Model, occlusionParams, child.Model:GetExtentsSize(), dataChannel)
		end
...
actor:BindToMessageParallel("occlusionCheck", function(model: Model, params: T.dict, boundingBox: CFrame, dataChannel: BindableEvent)
	...
	--Run occlusion check
	local parts = workspace:GetPartBoundsInBox(model.WorldPivot, boundingBox, overlapParams)
